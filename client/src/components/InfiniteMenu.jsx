...
function drawImageFramed(ctx, img, dx, dy, dWidth, dHeight) {
  const iw = img.naturalWidth || img.width
  const ih = img.naturalHeight || img.height

  if (!iw || !ih) return

  // Fill each atlas cell fully so circular discs don't show dark bands.
...
class InfiniteGridMenu {
  ...
  setPaused(paused) {
    ...
    // Reset timing baseline so resuming continues smoothly from the paused state.
  ...
  initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (img) {
          drawImageFramed(ctx, img, x, y, cellSize, cellSize)
        } else {
        ...
        <>
          <h2
            className={`
              ...
              max-w-[7.2ch]
              font-black
              text-[clamp(1.8rem,4.2vw,3.5rem)]
              leading-[0.98]
              left-[15%]
              top-1/2
              -translate-y-1/2
          ...
          <p
            className={`
              ...
              ${
                isMoving
                  ? 'opacity-0 pointer-events-none duration-[100ms] translate-x-6 -translate-y-1/2'
          ...
          <button
            ...
            className={`
              ...
              hover:scale-110
              hover:-translate-y-1
              hover:shadow-[0_0_30px_rgba(82,39,255,0.55)]
              active:scale-95
              ${
                isMoving
            ...
            `}
            aria-label="Open selected project on YouTube"
          >
            <span className="select-none relative top-[2px] text-[30px] text-white transition-transform duration-300 group-hover:translate-x-[2px] group-hover:-translate-y-[2px]">
              ↗
            </span>
          </button>
        </>
